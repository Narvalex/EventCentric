...
-- App
--------------------------------
insert into EventStore.Subscriptions(SubscriberStreamType, StreamType, Url, Token, ProcessorBufferVersion, IsPoisoned, WasCanceled, CreationLocalTime, UpdateLocalTime) values (
	N'OccClient1',
	N'OccClient1App',
	N'none', N'self', 0, 0, 0, getdate(), getdate())
if(@@error<>0) goto errorHandler
...
insert into EventStore.Subscriptions(SubscriberStreamType, StreamType, Url, Token, ProcessorBufferVersion, IsPoisoned, WasCanceled, CreationLocalTime, UpdateLocalTime) values (
	N'OccClient1',
	N'OccServer',
	N'http://localhost:51343/eventsource/events', N'123', 0, 0, 0, getdate(), getdate())
if(@@error<>0) goto errorHandler
